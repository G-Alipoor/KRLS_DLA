function res = OKDLRSTest(varargin)
...
%
% Examples:
%   res = OKDLRSTest('BatchSize',10, 'NMissing',4, 'nofTests',10);
%   res = OKDLRSTest('NMissing',5, 'nofTests',10);
%   res = OKDLRSTest('NMissing',2, 'nofTests',25, 'NFolds',2, 'KernelType','Poly', 'KernelParam',1.0, 'verbose',0);
%   res = OKDLRSTest();
%   res = OKDLRSTest('KernelType','Poly', 'KernelParam',1.0);
%
...
[DataSet, KernelType, KernelParam, Q, s, Rho, mu_m, BatchSize, nofBatches, maxL, newL, NMissing, NFolds, ...
...
resFileName = fullfile('..\Results\', sprintf('OKDLRS_%s_%s', DataSet, TestVersion));
...
if (exist([resFileName,'.mat'],'file') == 2)
    ...
    for f = 1:NFolds
        ...
        for c = 1:NClass
            ...
            Idx = 1:Q;
            P_i{c}.Y = XTrain(:, Idx, c);
            P_i{c}.K = ker_eval(P_i{c}.Y, P_i{c}.Y, KernelType, KernelParam);
            P_i{c}.C = eye(Q);
            P_i{c}.A = eye(Q);
        end
        % Test rhe initialized dictionaries
        tic
        for m = 1:NMissing
            if ~verbose
                fprintf('m =%2i',m);
            ...
            for i = 1:NClass
                ...
                for c = 1:NClass
                    ...
                    k = ker_eval(P_i{c}.Y, CurrXTest, KernelType, KernelParam);
                    h = P_i{c}.A' * k;
                    [~, r2] = myKOMP(P_i{c}.A'*P_i{c}.K*P_i{c}.A, h, Sig, s, 1e-6, false, false);
                    Err(c, :) = r2(:)';
                end
                [~, Prd(i, :)] = min(Err);
            end
        ...
        for b = 1:nofBatches
            ...
            for c = 1:NClass
                tic
                P_i{c} = OKDLRSUpdate(XTrain(:, Idx, c), P_i{c}, 'Rho',Rho, 'mu_m',mu_m, 'maxL',maxL, 'newL',newL);
...
if verbose
    ...
    title(sprintf('OKDLRS test on %s dataset',DataSet), 'FontSize',16)
    V = axis();
    V(3) = max( 0.8, V(3) );
    % v(4) = 0.98;
    axis(V);
...
if verbose
    ...
    fprintf('OKDLRS-DLA test of %s data, Q=%i, s=%i.\n', DataSet, Q, s)
